...
import AsyncAsset from '../AsyncAsset';
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700 rounded-xl overflow-hidden hover:border-slate-600 transition-colors flex flex-col md:flex-row h-auto min-h-[180px] group relative">
      ...
      <div className="w-full md:w-[100px] bg-black relative shrink-0 aspect-[9/16] md:aspect-[9/16] self-center md:self-auto md:ml-4 my-4 md:my-0 rounded-lg overflow-hidden border border-slate-800 shadow-lg">
        {segment.visualUrl ? (
          <AsyncAsset
            url={segment.visualUrl}
            type={(segment.visualType === 'video' || segment.visualType === 'avatar_video') ? 'video' : 'image'}
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          {menuOptions.map((opt, idx) => {
            ...
            return (
              <button
                key={idx}
                onClick={() => onOpenChat(opt.mode, segment)}
                className="flex items-center justify-center space-x-2 p-2 bg-slate-700/50 hover:bg-indigo-600/20 text-slate-300 hover:text-indigo-300 rounded-lg transition-all border border-transparent hover:border-indigo-500/30 text-xs"
                title={opt.label}
              >
                <Icon size={14} />
                <span className="truncate">{opt.label.replace('Create ', '')}</span>
              </button>
            );
          })}
        </div>
