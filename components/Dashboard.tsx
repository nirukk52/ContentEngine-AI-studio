...
import { TrendingUp, Clock, Users, Video, Zap, HardDrive, Calendar, ArrowRight, CheckCircle2, Folder, FileText } from 'lucide-react';
import { StoredProjectIndexEntry } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ projects, onSelectProject, onCreateProject }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-white px-1">Recent Projects (Drive Synced)</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {projects.map(p => (
            <button
              key={p.projectId}
              onClick={() => onSelectProject(p.projectId)}
              className="flex items-center gap-4 p-4 bg-slate-800/60 border border-slate-700/50 rounded-2xl hover:bg-slate-800 hover:border-indigo-500/50 transition-all text-left group"
            >
              <div className="p-3 bg-indigo-500/10 text-indigo-400 rounded-xl group-hover:scale-110 transition-transform">
                <Folder size={20} />
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-200 truncate">{p.name || "Untitled Project"}</h3>
                <p className="text-xs text-slate-500 mt-1">Edited {new Date(p.updatedAt).toLocaleDateString()}</p>
              </div>
            </button>
          ))}
          ...
          <button
            ...
            className="flex items-center gap-4 p-4 bg-slate-800/30 border border-dashed border-slate-700 rounded-2xl hover:bg-slate-800 hover:border-indigo-500/50 transition-all text-left text-slate-400 hover:text-indigo-400"
          >
        ...
        {projects.length === 0 && (
          <div className="text-center py-12 text-slate-500 text-sm">
            No projects found in Drive. Start a new one!
          </div>
        )}
      </div>

      {/* Drive & Production Management (Legacy / Quick Actions) */}
